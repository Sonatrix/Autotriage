...
from autotriage.models import Ticket, FollowUp, Queue

User = get_user_model()


class OpenTicketsByUser(Feed):
    title_template = 'autotriage/rss/ticket_title.html'
    description_template = 'autotriage/rss/ticket_description.html'
    ...
    def title(self, obj):
        if obj['queue']:
            return _("Autotriage: Open Tickets in queue %(queue)s for %(username)s") % {
                'queue': obj['queue'].title,
                'username': obj['user'].get_username(),
            }
        else:
            return _("Autotriage: Open Tickets for %(username)s") % {
                'username': obj['user'].get_username(),
            }
...
class UnassignedTickets(Feed):
    ...
    title = _('Autotriage: Unassigned Tickets')
    description = _('Unassigned Open and Reopened tickets')
    link = ''  # '%s?assigned_to=' % reverse('autotriage:list')
...
class RecentFollowUps(Feed):
    title_template = 'helpautotriagedesk/rss/recent_activity_title.html'
    description_template = 'autotriage/rss/recent_activity_description.html'

    title = _('Autotriage: Recent Followups')
    description = _('Recent FollowUps, such as e-mail replies, comments, attachments and resolutions')
    link = '/tickets/'  # reverse('autotriage:list')
...
class OpenTicketsByQueue(Feed):
    ...
    def title(self, obj):
        return _('Autotriage: Open Tickets in queue %(queue)s') % {
            'queue': obj.title,
        }
