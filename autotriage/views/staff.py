...
def dashboard(request):
    ...
    from_clause = """FROM    autotriage_ticket t,
                    autotriage_queue q"""
    if queues:
    ...
    return render(request, 'autotriage/dashboard.html', {
...
def delete_ticket(request, ticket_id):
    ...
    if request.method == 'GET':
        return render(request, 'autotriage/delete_ticket.html', {
            'ticket': ticket,
        })
    else:
        ticket.delete()
        return HttpResponseRedirect(reverse('autotriage:home'))
...
def followup_edit(request, ticket_id, followup_id):
    ...
    if request.method == 'GET':
        ...
        return render(request, 'autotriage/followup_edit.html', {
            'followup': followup,
            'ticket': ticket,
            'form': form,
        ...
        if form.is_valid():
            ...
            # delete old followup
            followup.delete()
        return HttpResponseRedirect(reverse('autotriage:view', args=[ticket.id]))
...
def view_ticket(request, ticket_id):
    ...
    return render(request, 'autotriage/ticket.html', {
        'ticket': ticket,
        'form': form,
        'active_users': users,
...
def update_ticket(request, ticket_id, public=False):
    if not (public or (
        ...
        return HttpResponseRedirect('%s?next=%s' %
                                    (reverse('autotriage:login'), request.path))
    ...
    if ticket.assigned_to and \
        ...
        if (not reassigned or
                (reassigned and
                    ticket.assigned_to.usersettings_autotriage.settings.get(
...
def ticket_list(request):
    ...
    if request.GET.get('saved_query', None):
        ...
        import json
        from autotriage.lib import b64decode
        try:
            if six.PY3:
    ...
    import json
    from autotriage.lib import b64encode
    urlsafe_query = b64encode(json.dumps(query_params).encode('UTF-8'))

    user_saved_queries = SavedSearch.objects.filter(Q(user=request.user) | Q(shared__exact=True))

    return render(request, 'autotriage/ticket_list.html', dict(
        context,
        tickets=ticket_qs,
        default_tickets_per_page=request.user.usersettings_autotriage.settings.get(
            'tickets_per_page') or 25,
...
def edit_ticket(request, ticket_id):
    ...
    return render(request, 'autotriage/edit_ticket.html', {'form': form})
...
def create_ticket(request):
    ...
    if request.method == 'POST':
        ...
        initial_data = {}
        if request.user.usersettings_autotriage.settings.get('use_email_as_submitter', False) and request.user.email:
    ...
    return render(request, 'autotriage/create_ticket.html', {'form': form})
...
def rss_list(request):
    return render(request, 'autotriage/rss_list.html', {'queues': Queue.objects.all()})
...
def report_index(request):
    ...
    return render(request, 'autotriage/report_index.html', {
...
def run_report(request, report):
    if Ticket.objects.all().count() == 0 or report not in (
        ...
        return HttpResponseRedirect(reverse("autotriage:report_index"))
    ...
    return render(request, 'autotriage/report_output.html', {
        'title': title,
        'charttype': charttype,
        'data': table,
...
def save_query(request):
    ...
    return HttpResponseRedirect('%s?saved_query=%s' % (reverse('autotriage:list'), query.id))
...
def delete_saved_query(request, id):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'autotriage/confirm_delete_saved_query.html', {'query': query})
delete_saved_query = staff_member_required(delete_saved_query)


def user_settings(request):
    s = request.user.usersettings_autotriage
    if request.POST:
    ...
    return render(request, 'autotriage/user_settings.html', {'form': form})
user_settings = staff_member_required(user_settings)


def email_ignore(request):
    return render(request, 'autotriage/email_ignore_list.html', {
...
def email_ignore_add(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return HttpResponseRedirect(reverse('autotriage:email_ignore'))
    else:
        form = EmailIgnoreForm(request.GET)

    return render(request, 'autotriage/email_ignore_add.html', {'form': form})
...
def email_ignore_del(request, id):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'autotriage/email_ignore_del.html', {'ignore': ignore})
...
def ticket_cc(request, ticket_id):
    ...
    return render(request, 'autotriage/ticket_cc_list.html', {
        'copies_to': copies_to,
        'ticket': ticket,
    })
...
def ticket_cc_add(request, ticket_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            ticketcc.save()
            return HttpResponseRedirect(reverse('autotriage:ticket_cc',
                                                kwargs={'ticket_id': ticket.id}))
    else:
        form_email = TicketCCEmailForm()
        form_user = TicketCCUserForm()
    return render(request, 'autotriage/ticket_cc_add.html', {
        'ticket': ticket,
...
def ticket_cc_del(request, ticket_id, cc_id):
    ...
    return render(request, 'autotriage/ticket_cc_del.html', {'cc': cc})
...
def ticket_dependency_add(request, ticket_id):
    ...
    return render(request, 'autotriage/ticket_dependency_add.html', {
        'ticket': ticket,
        'form': form,
    })
...
def ticket_dependency_del(request, ticket_id, dependency_id):
    ...
    if request.method == 'POST':
        dependency.delete()
        return HttpResponseRedirect(reverse('autotriage:view', args=[ticket_id]))
    return render(request, 'autotriage/ticket_dependency_del.html', {'dependency': dependency})
...
def attachment_del(request, ticket_id, attachment_id):
    ...
    if request.method == 'POST':
        attachment.delete()
        return HttpResponseRedirect(reverse('autotriage:view', args=[ticket_id]))
    return render(request, 'autotriage/ticket_attachment_del.html', {
