...
from autotriage.models import Attachment, EmailTemplate
...
def send_templated_mail(template_name,
    ...
    from autotriage.models import EmailTemplate
    from autotriage.settings import HELPDESK_EMAIL_SUBJECT_TEMPLATE, \
    ...
    footer_file = os.path.join('autotriage', locale, 'email_text_footer.txt')
    ...
    email_html_base_file = os.path.join(
        'autotriage', locale, 'email_html_base.html')
...
def text_is_spam(text, request):
    ...
    try:
        from autotriage.akismet import Akismet
    except:
        return False
    try:
